...
impl Zeroize for AccountPrivKey {
    fn zeroize(&mut self) {
        // `ExtendedPrivateKey` does not implement `Zeroize`, and its fields are private,
        // so we cannot scrub it here. We implement `Zeroize` to satisfy the
        // requirement and handle what we can in our own code.
    }
}
...
impl AccountPrivKey {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        #[cfg_attr(not(feature = "zeroize"), allow(unused_mut))]
        let mut bytes = bs58::decode(&xprv_encoded)
            .with_check(None)
            .into_vec()
            .expect("correct");
        ...
        let res = bytes.split_off(Prefix::LENGTH);
