...
impl AccountPrivKey {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        #[cfg(feature = "zeroize")]
        {
            ...
            // The original vector retains its allocated capacity; to prevent leaks of the
            // discarded data, we must manually set its length to its capacity before
            // zeroizing it.
            unsafe {
                let len = decoded.capacity();
                decoded.set_len(len);
            }
            decoded.zeroize();
        }
