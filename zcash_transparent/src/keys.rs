...
impl AccountPrivKey {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        #[cfg(feature = "zeroize")]
        {
            ...
            // We can't easily zeroize the truncated `decoded` vector's original
            // prefix data because `split_off` returns a new vector and the
            // original one's data might be left in memory.
            // However, we can zeroize `decoded` itself.
            decoded.zeroize();
        }
